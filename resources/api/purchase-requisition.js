...
GET('purchase-requisition/:id'          , () => {
    ...
    validate(data, rule, () => {
        var pr 			= PR.select('purchase_requisitions.id', 'number', 'remarks', 'status', 'processed_date', 'approved_date', 'cancelled_date', 'purchase_requisitions.created_at', 'purchase_requisitions.updated_at', 'processors.username as processed_by', 'approvers.username as approved_by', 'cancellers.username as cancelled_by', 'creators.username as created_by', 'updaters.username as updated_by')
			...
			prDetails 	= PRD 	.select('purchase_requisition_details.id', 'quantity', 'target_date', 'purchase_requisition_details.created_at', 'purchase_requisition_details.updated_at', 'product_code', 'products.description', 'location_code')
...
GET('purchase-requisitions' 			, () => {
    ...
    foreach(pr.get(), (indexPR, eachPR) => {
        ...
        details	.select('purchase_requisition_details.id', 'quantity', 'target_date', 'purchase_requisition_details.created_at', 'purchase_requisition_details.updated_at', 'product_code', 'products.description', 'location_code')
				.leftJoin('products', 'products.id', 'product_id')
				.leftJoin('locations', 'locations.id', 'location_id')
